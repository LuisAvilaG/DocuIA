...
import { subsidiaries, catalogItems, catalogVendors, catalogLocations, orgFeatures } from "@/db/schema";
...
async function handleGET(req: NextRequest) {
  ...
  try {
    ...
    for (const org of orgs) {
      // One organization's bad credentials or data must not stop the run
      // for every organization after it.
      try {
        let feat;
        try {
        ...
        const conn = await getActiveNsConnection(org.id);
        ...
        for (const sub of subs) {
          ...
          const subSummary: Record<string, number | string> = {};
          ...
          for (const type of syncTypes) {
            ...
            while (true) {
              ...
              if (!result.ok || !result.data) {
                // Report partial pages instead of a silent "ok".
                if (!result.ok) subSummary[`${type}_error`] = result.error ?? "fetch failed";
                break;
              }
        ...
        console.error("[cron/auto-sync] org failed", org.id, err);
        summary[org.id] = { error: err instanceof Error ? err.message : "sync failed" };
      }
    }
