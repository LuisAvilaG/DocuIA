        ...
        <form onSubmit={saveMapping} className="border-b border-border bg-card px-6 py-5">
          <div className="max-w-5xl">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              <label className="relative space-y-1.5 text-xs text-muted-foreground">Buscar ítem del ERP<div className="relative mt-1.5"><Database className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><input value={itemQuery} onChange={event => searchCatalogItems(event.target.value)} placeholder="Nombre o código de ítem" className="w-full rounded-md border border-border bg-background py-2 pl-9 pr-8 text-sm text-foreground outline-none focus:border-primary" />{searchingItems && <Loader2 className="absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 animate-spin text-muted-foreground" />}</div>{suggestions.length > 0 && <div className="absolute z-20 mt-1 max-h-44 w-full overflow-auto rounded-md border border-border bg-card py-1 shadow-md">{suggestions.map(item => <button type="button" key={item.internalId} onClick={() => selectCatalogItem(item)} className="block w-full px-3 py-2 text-left text-xs transition-colors hover:bg-secondary"><span className="block font-medium text-foreground">{item.name || item.itemid}</span><span className="text-muted-foreground">#{item.internalId}{item.itemid ? ` · ${item.itemid}` : ""}</span></button>)}</div>}</label>
              <label className="space-y-1.5 text-xs text-muted-foreground">ID del ítem en el ERP<input required value={editor.netsuiteInternalId} onChange={event => setEditor({ ...editor, netsuiteInternalId: event.target.value })} placeholder="Ej. 123" className="mt-1.5 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary" /></label>
              <label className="space-y-1.5 text-xs text-muted-foreground">Nombre en el ERP<input value={editor.netsuiteItemName} onChange={event => setEditor({ ...editor, netsuiteItemName: event.target.value })} placeholder="Se completa al elegir del catálogo" className="mt-1.5 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary" /></label>
            </div>
