...
export async function processContractCase(caseId: string, deps: ContractExtractDeps = realExtractDeps): Promise<void> {
  ...
  try {
    ...
    // Calculations are deterministic, never generated by the model. They run
    // after extraction so every output can be traced to an extracted base.
    const calculations = evaluateCalculations(plan.calculations, docsByType);
    const calculationValues = Object.fromEntries(calculations.map((item) => [item.key, item.value]));
